...
\section{EVALUATION}
We conducted an experiment to test the performance of Peapods.The experimental machine has an Intel Core i7-4770S quad-core processor running a Linux operating system (kernel version 3.13.1).The maximum frequency of  processor is 3.4GHz.We experimented with the PolarSSL cryptographic algorithm library and used Peapods to protect 2048-bit RSA private key calculations after splitting into 128 transactions and 256 transactions.The comparison objects in the experiment include: (1) the official default configuration of PolarSSL (version 1.3.6), (2) Peapods\_128, the Peapods split into 128 transactions,(3) Peapods\_256, the Peapods split into 256 transactions.
...
We used Geekbench 3 to test Peapods' impact on other concurrent processes.Figure\ref{fig:stream} shows Geekbench 3 scores in multi-core mode and single-core mode.The baseline is a score in clean environment.
...
{\color{blue}Intel Software Guard eXtensions (SGX) provide a hardware-enabled secure container that is isolated from other processes.We evaluated the performance of unmodified polarssl RSA decryption under SGX protection with only one percent performance overhead. This result may be caused by the absence of frequent entry and exit enclave during rsa decryption. Since the TLB caches the memory access in the enclave, the entry and exit of the enclave requires TLB flush, so the entry and exit of the enclave in SGX is the bottleneck.In addition, sgx has the following disadvantages:1.SGX requires developers to refactor the code and divide the program into trusted and non-trusted parts. Although there are currently SDKs released by Intel to assist in this work, it is still a lot of work, and it is easier to cause a secret leak. 2.If the enclave code itself has vulnerabilities, the enclave is unable to protect the integrity of the program. Currently, the ROP attack against the bufferoverflow can control the enclave\cite{lee2017hacking}. There are also some side-channel attacks that can cause secret leakage\cite{brasser2017software}. 3.SGX requires additional hardware support, such as the motherboard needs to be adapted to the intel CPU.}
