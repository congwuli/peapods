...
The cryptographic software system is linked to the user's core interests. When users use online transactions, e-mail, and remote login services, they rely on the protection of cryptographic software systems. Once the security of cryptographic software systems is not guaranteed, it will cause extreme problems for users.On the other hand, the speed of password computing directly affects the user's experience with the crypto software system. Therefore, it is imperative to provide secure, high-speed and reliable cryptographic services.
...
\item Implementation is independent of the cryptographic algorithm。

\end{enumerate}

The first and second goals can resist software memory attacks, the third goal is to make a successful cold-boot attack can only obtain sensitive information of the ciphertext, the fourth goal is to enable Peapods to implement security enhancements for a variety of cryptographic software systems.

In summary, Peapods can resist various software memory attacks caused by operating system kernel vulnerabilities or application software vulnerabilities.At the same time, as sensitive data is never written to the DRAM chip during the calculation, Peapods can also resist cold-boot attacks and provide security enhancements for various cryptographic software systems.
